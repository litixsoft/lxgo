...
func Send(s Sender, msg ...*Message) error {
	for i, m := range msg {
		if err := send(s, m); err != nil {
			return fmt.Errorf("mail: could not send email %d: %v", i+1, err)
		}
	}
...
func (m *Message) getFrom() (string, error) {
	...
	if len(from) == 0 {
		from = m.header["From"]
		if len(from) == 0 {
			return "", errors.New(`mail: invalid message, "From" field is absent`)
		}
	}
...
func parseAddress(field string) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("mail: invalid address %q: %v", field, err)
	}
